...
incomelevel <- c("under 20", "20 to 40", "40 to 60", "60 to 80", "80 to 100", "100 to 120", "120 to 140", "140 or more")
...
The boxplot indicates that around half of the families with household income of 80000 or more have more than 1 freezer. The majority of the richest families with household income of 140000 or more have more than 1 freezer.
```{r}
boxplot(use, ylab="number of freezer", xlab="household income")
```

## histogram: number of refrigerator vs household income

The histogram indicates the number of freezers own by family with different household income. All the families that do not use freezer have household income under 20000.
```{r}
ggplot(data = use1, aes(x= income, y = household, fill= number.of.freezer)) + geom_bar(stat="identity")
```
## Summary: number of refrigerator owned by population breaking down by household income

Very limited (less than 1%) homes in the US do not have a refrigerators and they come from household with lowest income. The majority of American homes have only 1 refrigerator. As household income increases, the percentage of having more than 1 freezer in each income level increases. There is a jump between income levels of 60,000 to 80,000 and 80,000 to 100,000. Nearly half of homes with household income of 80,000 to 140,000 have more than 1 refrigerator. More than half of homes(54.5%) with income of 140,000 or more have at least 2 freezers. Thus, the second refrigerator should mainly target at homes with more than 80,000 household income.
...
```{r}
use <- read.csv("size2.csv", header = TRUE)##### ERROR! I NEED TO SUBMIT THE SIZE2 CSV FILE
summary(size2)
```
...
sizeorder<-c('Half-size or compact','Small (less than 17.6 cubic feet)','Medium (17.6 to 22.5 cubic feet)','Large (22.6 to 29.5 cubic feet)','Very large (more than 29.5 cubic feet)','Do not use a refrigerator')
...
Two doors, top freezer and two doors, side by side freezer are the most commonly used refrigerator types by majority of all households.Thus, marketing should be focusing on two doors, top freezer and two doors, side by side freezer. households of less than 20,000 income and households with 20,000-40,000 (median income families) have the most number of refrigerators, which indicates marketing should be focusing on these two groups of households.Half-size/compact regrigerators are only used by low income families. Companies wishing to sell half-size/compact refrigerators may focus on low income families by reducing the price of the products. 
...
colnames(age) <- c('Age','Under 20', '20 to 40', '40 to 60', '60 to 80', '80 to 100', '100 to 120', '120 to 140', '140+')
...
```{r}
incomeorder <- c('Under 20', '20 to 40', '40 to 60', '60 to 80', '80 to 100', '100 to 120', '120 to 140', '140+')
ageorder <- c('Less than 2 years old', '2 to 4 years old', '5 to 9 years old', '10 to 14 years old', '15 to 19 years old', '20 or more years old', 'Do not use a refrigerator')
...
```{r}
boxplot(age[, 2:8], xlab = "Income bracket (in thousands of dollars)", ylab = "Units (in thousands)")
```
